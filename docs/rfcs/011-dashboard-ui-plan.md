...
**Spec:** `docs/rfcs/011-dashboard-ui.md`
...
Same structure as LeadTimeChart but with different title. To keep DRY, create a shared percentile chart:
...
Note: LeadTimeChart and CycleTimeChart are intentionally separate files despite identical structure — they'll likely diverge as the product evolves (different scales, annotations, etc). If they stay identical long-term, refactor into a shared `PercentileChart`.
