...
- **Setup Configuration** — Whether the pipeline is ready. Checks if a production environment is configured, which is required for deployment-based metrics. An environment qualifies if its GitHub name contains `prod` or `live`, or if it has been marked production manually.
