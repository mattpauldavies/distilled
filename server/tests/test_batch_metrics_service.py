...
async def test_deployment_frequency_counts_by_date(mock_session):
    ...
    from app.services.batch_metrics_service import compute_deployment_frequency
...
async def test_lead_time_computes_median_and_p75(mock_session):
    ...
    from app.services.batch_metrics_service import compute_lead_time
...
async def test_pr_cycle_time_computes_from_opened_to_merged(mock_session):
    ...
    from app.services.batch_metrics_service import compute_pr_cycle_time
...
async def test_pr_throughput_counts_by_week(mock_session):
    ...
    from app.services.batch_metrics_service import compute_pr_throughput
...
async def test_recompute_repo_calls_all_four_metrics(mock_session):
    ...
    from app.services.batch_metrics_service import recompute_repo

    with (
        patch("app.services.batch_metrics_service.compute_deployment_frequency", new_callable=AsyncMockFn) as mock_df,
        patch("app.services.batch_metrics_service.compute_lead_time", new_callable=AsyncMockFn) as mock_lt,
        patch("app.services.batch_metrics_service.compute_pr_cycle_time", new_callable=AsyncMockFn) as mock_ct,
        patch("app.services.batch_metrics_service.compute_pr_throughput", new_callable=AsyncMockFn) as mock_tp,
    ):
...
async def test_recompute_repo_continues_on_partial_failure(mock_session):
    ...
    with (
        patch(
            "app.services.batch_metrics_service.compute_deployment_frequency",
            new_callable=AsyncMockFn,
            side_effect=exc,
        ),
        patch("app.services.batch_metrics_service.compute_lead_time", new_callable=AsyncMockFn) as mock_lt,
        patch("app.services.batch_metrics_service.compute_pr_cycle_time", new_callable=AsyncMockFn) as mock_ct,
